...
import { MapPin, Calendar, Bell, Phone, Clock, AlertCircle, CheckCircle, Navigation } from "lucide-react"
...
export default function ParentHomePage() {
  ...
  const busStatus = {
    status: "moving_to_pickup", // moving_to_pickup, at_school, moving_home, delayed, incident
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className="text-right">
                <p className="text-2xl font-bold text-foreground">{currentTime.toLocaleTimeString("vi-VN")}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          <Link href="/parent/tracking">
            <Button
              variant="outline"
              className="w-full h-auto flex-col gap-2 py-4 hover:bg-primary/5 hover:border-primary bg-transparent"
            >
          ...
          <Link href="/parent/messages">
            <Button
              variant="outline"
              className="w-full h-auto flex-col gap-2 py-4 hover:bg-orange-500/5 hover:border-orange-500 bg-transparent"
            >
        ...
        <Card className="border-0 shadow-md">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/parent/messages" className="text-sm text-primary hover:underline">
                Xem tất cả
              </Link>
            </div>
            <div className="space-y-3">
              {notifications.map((notification) => (
                ...
                >
                  <Bell
                    className={`w-4 h-4 mt-0.5 ${notification.unread ? "text-primary" : "text-muted-foreground"}`}
                  />
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm ${notification.unread ? "font-medium text-foreground" : "text-muted-foreground"}`}
                    >
