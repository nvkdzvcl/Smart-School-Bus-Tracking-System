...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    // (Optional) đọc theme lưu trước đó
    ...
    if (savedTheme) {
      ...
      // nếu bạn muốn default dark
      document.documentElement.classList.toggle("dark", true)
    }
  }, [navigate])

  // Sync dark mode với <html class="dark">
  useEffect(() => {
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-foreground">{driverName || "Chưa có tên"}</h3>
                <p className="text-sm text-muted-foreground">{driverPhone || "Chưa có số điện thoại"}</p>
